...
public class SwiftService {
    ...
    private final int SWIFT_CODE_LENGTH = 11;
    ...
    public ResponseSwiftEntryDto createNewSwiftEntry(RequestBankDto requestDto){
        ...
        if(country.isEmpty()) return new ResponseSwiftEntryDto("Entry addition failed due to incorrect countryISO2 code");
        if(requestDto.swiftCode() == null) return new ResponseSwiftEntryDto("Entry addition failed due to missing SWIFT code");
        if(!isSwiftCodeValid(requestDto.swiftCode())) return new ResponseSwiftEntryDto("Entry addition failed due to invalid SWIFT code");
        if(requestDto.swiftCode().contains("XXX")){
                if(!requestDto.isHeadquarter()) return new ResponseSwiftEntryDto("Entry addition failed due to a discrepancy in SWIFT code logic");
        } else {
            if(requestDto.isHeadquarter()) return new ResponseSwiftEntryDto("Entry addition failed due to a discrepancy in SWIFT code logic");
        }
        ...
        try {
            ...
            return new ResponseSwiftEntryDto("Entry addition successful");
        } catch (Exception e){
            return new ResponseSwiftEntryDto("Entry addition failed. Error message: " + e.getMessage());
        }
    }

    public ResponseSwiftEntryDto deleteSwiftEntry(String swiftCode){
        Optional<Bank> bank = bankRepository.findBankBySwift(swiftCode);
        if(bank.isEmpty()){
            return new ResponseSwiftEntryDto("Entry deletion failed due to SWIFT not being found");
        }
        try {
            bankRepository.delete(bank.get());
            return new ResponseSwiftEntryDto("Entry deletion successful");
        } catch (Exception e){
            return new ResponseSwiftEntryDto("Entry deletion failed. Error message: " + e.getMessage());
        }
    }
    ...
    private boolean isSwiftCodeValid(String swiftCode){
        return swiftCode.strip().length() == SWIFT_CODE_LENGTH;
    }
}
